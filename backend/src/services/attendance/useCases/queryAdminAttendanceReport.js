...
function toIsoRange({ toIsoDate, range }) {
  return {
...
async function aggregateHolatStats({ prisma, normalizeHolatCounts, calcFoizFromCounts, baseWhere, range }) {
...
function buildAdminAttendanceFilterSql({
  Prisma,
  selectedRange,
  classroomId,
  studentId,
...
async function estimateExpectedAttendanceTotals({
  ...
  const { from: fromIso, toExclusive: toIsoExclusive } = toIsoRange({
    toIsoDate,
    range: selectedRange,
  });
...
async function executeGetAdminAttendanceReportCore({ deps, query = {}, options = {} }) {
  const {
    Prisma,
    prisma,
    parseSanaOrToday,
...
async function executeGetAdminAttendanceReportData({ deps, query = {} }) {
  const { toIsoDate } = deps;
  const core = await executeGetAdminAttendanceReportCore({ deps, query });
