...
export default function StudentSchedulePage() {
  ...
  const [fetchStudentSchedule] = useLazyGetStudentScheduleQuery();

  const loadSchedule = useCallback(async (nextYear = '') => {
    setLoading(true);
    setError('');
    try {
      const data = await fetchStudentSchedule(nextYear ? { oquvYili: nextYear } : {}).unwrap();
