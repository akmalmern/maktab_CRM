...
export default function PersonDetailPage() {
  ...
  async function handleResetPassword() {
    const newPassword = window.prompt('Yangi parol kiriting (kamida 8 ta belgi):', '');
    ...
    const ok = await askConfirm(
      "Kiritilgan yangi parol saqlansinmi? Eski parol endi ishlamaydi.",
      "Parolni yangilash",
    );
    if (!ok) return;
    ...
    if (resetPersonPasswordThunk.fulfilled.match(result)) {
      toast.success(result.payload?.message || "Parol yangilandi");
      return;
    }

    toast.error(result.payload || "Parol yangilanmadi");
  }
    ...
    <AutoTranslate>
      ...
      {activeTab === 'profile' && (
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <Card className="lg:col-span-2" title="Profil ma'lumotlari">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <p><span className="font-semibold">Username:</span> {person.user?.username || '-'}</p>
              <div className="md:col-span-1">
                <Button size="sm" variant="indigo" onClick={handleResetPassword} disabled={actionLoading}>
                  Parolni yangilash
                </Button>
              </div>
              <p><span className="font-semibold">Telefon:</span> {person.user?.phone || '-'}</p>
              <p><span className="font-semibold">Manzil:</span> {person.yashashManzili || '-'}</p>
              <p><span className="font-semibold">Tug`ilgan sana:</span> {formatDate(person.birthDate)}</p>
              {type === 'teacher' && <p><span className="font-semibold">Fan:</span> {person.subject?.name || '-'}</p>}
              {type === 'student' && (
                <>
                  <p>
                  ...
                  <p><span className="font-semibold">Ota-ona telefoni:</span> {person.parentPhone || '-'}</p>
                </>
              )}
            </div>
          </Card>
      ...
      {activeTab === 'documents' && (
        <Card title="Hujjatlar">
          <form onSubmit={handleUploadDocument} className="mt-1 grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 md:grid-cols-4">
        ...
        <Card title="Faoliyat">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Profil yaratilgan sana</p>
              <p className="mt-1 text-sm font-semibold text-slate-900">{formatDate(person.createdAt || person.user?.createdAt)}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Jami hujjatlar</p>
              <p className="mt-1 text-sm font-semibold text-slate-900">{person.documents?.length || 0} ta</p>
            </div>
          </div>
        </Card>
      )}
