...
function StatusPill({ value }) {
  ...
  return (
    <span className={`inline-flex rounded-full border px-2 py-0.5 text-xs font-semibold ${map[value] || map.DRAFT}`}>
...
export default function TeacherPayrollPage() {
  ...
  const payslipColumns = useMemo(
    () => [
      ...
      { key: 'adjustmentAmount', header: t('Tuzatma'), render: (row) => formatMoney(row.adjustmentAmount) },
  ...
  return (
    <div className="space-y-4">
      <Card
        ...
        subtitle={t("Hisob-kitoblar bo'yicha oylik va satr tafsilotlarini ko'ring")}
        actions={(
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            ...
            <Select value={query.status} onChange={(e) => setQuery((prev) => ({ ...prev, status: e.target.value, page: 1 }))}>
              ...
              <option value="DRAFT">{getPayrollStatusLabel('DRAFT', t)}</option>
              <option value="APPROVED">{getPayrollStatusLabel('APPROVED', t)}</option>
              <option value="PAID">{getPayrollStatusLabel('PAID', t)}</option>
              <option value="REVERSED">{getPayrollStatusLabel('REVERSED', t)}</option>
            </Select>
      ...
      <Card
        title={t("Hisob-varaq tafsiloti")}
        subtitle={payslipDetail?.payrollRun?.periodMonth || t("Hisob-kitob tanlanmagan")}
      ...
      >
        {!selectedRun && <StateView type="empty" description={t("Hisob-varaqni tanlang")} />}
        ...
        {payslipDetail && !payslipDetailQuery.isLoading && !payslipDetailQuery.error && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="text-xs text-slate-500">{t('Holat')}</div><div className="mt-1"><StatusPill value={payslipDetail.payrollRun?.status} /></div></div>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="text-xs text-slate-500">{t('Tuzatma')}</div><div className="mt-1 font-semibold">{formatMoney(payslipDetail.adjustmentAmount)}</div></div>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-600">
               <div>{t('Jami satr')}: {payslipDetailQuery.data?.lines?.total || 0}</div>
