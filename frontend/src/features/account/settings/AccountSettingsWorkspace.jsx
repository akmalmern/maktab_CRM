...
export default function AccountSettingsWorkspace({ scope, roleLabel }) {
  const vm = useAccountSettingsController({ scope, roleLabel });
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[320px_minmax(0,1fr)]">
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            <div className="mx-auto flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border border-slate-300 bg-white text-2xl font-bold text-slate-600 shadow-inner">
              {vm.avatarUrl ? (
                <img src={vm.avatarUrl} alt="profile avatar" className="h-full w-full object-cover" />
              ) : (
                vm.initials
              )}
            </div>
